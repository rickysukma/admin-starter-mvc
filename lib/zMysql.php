...
global  $owlPDO;    
$query = $owlPDO->query("select * from ".$dbname.".".$table." limit 1");
$jlh_Kolom=$query->columnCount();
$primary = array();
    for($x=0;$x<$jlh_Kolom;$x++){
            $test=$query->getColumnMeta($x);
 ...
 function getEnum($dbname,$table,$field) {
       global  $owlPDO;    
       $query = $owlPDO->query(" SHOW COLUMNS FROM `".$dbname."`.`".$table."` LIKE '".$field."' ");
...
function getOrgBelow($dbname,$org,$self=true,$mode='all',$empty=false) {
  ...
  * O : total baris
  */
 function getTotalRow($dbname,$table,$where=null) {
...
/**
 * colDefaultValue
 * Mengambil default value dari kolom tertentu pada tabel tertentu
 * @param 	string	$dbname		DB Name
 * @param	string	$table		Table Name
 * @param	string	$column		Column Name
 * @return	int/string			Default Value
 */
function colDefaultValue($dbname, $table, $column) {
	$query = "SELECT DEFAULT( ".$column." ) AS def FROM ".$dbname.".".$table." LIMIT 1";
	$res = fetchData($query);
    return $res[0]['def'];
}